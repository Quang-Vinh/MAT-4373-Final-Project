...
X_test = matrix(NA,784,0)
Y_test = matrix(NA,10,0)
for(i in 0:9){
  X_tmp = t(M[sprintf("test%d",i)][[1]])
  X_test = cbind(X_test,X_tmp)
  Y_tmp = matrix(0,10,dim(X_tmp)[2])
  Y_tmp[i+1,] = 1
  Y_test = cbind(Y_test,Y_tmp)
}
...
{ 
  ...
  for (j in (1:ep))
  {
    ...
    for (i in seq(1, n, size))
    {
      ...
      Xtemp <- X[,indexes]
      Ytemp <- Y[,indexes]
      Xtesting <- Xtest[,indexes]
      Ytesting <- Ytest[,indexes]
      #estimate gradient
  ...
  return(list(W,lossbatch,testloss))
}
MBGDtest<-MBGD(X=X, Y=Y, size=50,rate=0.01,ep=10)
#Training
plot(MBGDtest[[2]])
#Testing
plot(MBGDtest[[3]])
